...
use crate::source::LuaSourceConfig;
...
async fn run_telegram_bot() -> AppResult<()> {
    ...
    let source_config = Arc::new(LuaSourceConfig::from_env());
    let http_client = build_http_client()?;
    let bot = Bot::with_client(telegram_token()?, http_client.clone());

    Dispatcher::builder(bot, schema())
        .dependencies(dptree::deps![lang_store, source_config, http_client])
...
fn build_http_client() -> AppResult<reqwest::Client> {
    Ok(reqwest::Client::builder()
        .user_agent(concat!("KiwiLuaBot/", env!("CARGO_PKG_VERSION")))
